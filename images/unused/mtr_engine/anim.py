...
class Anim(object):
    ...
    def set_anim(self, value, loop=True, reset=True, pingpong=False):
        self.kill = False
        self.loop = loop
        ...
        while self.count < 0:
            ...
            if self.index >= len(self.anim):
                if self.loop:
                    ...
                    self.kill = True
                    return
            self.cur_frame = self.anim[self.index][0] #frame index
            self.count += self.anim[self.index][1]    #delay ticks
