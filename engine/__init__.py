...
<map version="1.2" tiledversion="2020.02.12" orientation="orthogonal" renderorder="right-down" compressionlevel="0" width="''' + str(mwidth) + '''" height="''' + str(mheight) + '''" tilewidth="''' +str(twidth) + '''" tileheight="''' + str(theight) +   '''" infinite="0" nextlayerid="''' + str(ika.Map.layercount + 1) + '''" nextobjectid="1">
<tileset firstgid="1" source="''' + tilesetName + '''"/> '''
    ...
    for l in range(ika.Map.layercount):                
        ...
        for y in range(int(ika.Map.height/theight)):
            for x in range(int(ika.Map.width/twidth)):
